...
This is production-only and operator-node-only. It accepts no SHA, ref, workflow, or mode: the adapter replays the deployed app pin and the existing workflow consumes the merged infrastructure from `main`. The deployment parent is environment-scoped (`NODE_SUBMODULE_PARENT_{OWNER,REPO}`): candidate/test App operations stay in `cogni-test-org`; production App operations stay in `Cogni-DAO`. A `403 authz_denied` currently requires the same one-time production-promoter approval. That role reuse is a two-phase bootstrap bridge—the infra lever must first deploy the OpenFGA model before story.5028's dedicated `production_infra_promoter → can_reconcile_production_infra` grant can take over. Never replace it with personal `gh workflow run … -f skip_infra=false` or an SSH mutation. For node app placement/scale, use the Akash compute API instead; this verb owns only the shared state/edge substrate.
