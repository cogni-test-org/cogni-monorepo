...
| Base RPC is live                       | `GET /readyz?deep=1` forces a live Base RPC read and returns 200; `GET /api/v1/public/treasury/snapshot` returns `staleWarning:false` with `blockNumber>0`. Missing `EVM_RPC_URL` is fatal, while a transient vendor failure on default `/readyz` remains non-draining and emits `substrate.evm_rpc.unreachable`.                                                                                                                                                                                                                    | `pass/blocked` |
