...
describe("insertSchedulerEndpoint", () => {
  ...
  // bug.5094 — a formation PR splices the shared base configmap AND each deploy
  // env's generated node-endpoints.patch.yaml. The patch file is a bare ConfigMap
  // with the same 2-space-indented quoted line, and a birth is always k3s, so the
  // identical transform must hold there. Splicing base alone would birth a
  // drift-red PR (render-scheduler-worker-endpoints.sh --check) and an unrouted node.
  it("splices the generated per-env overlay patch the same way (bug.5094)", () => {
    const patchBefore = `# GENERATED by scripts/ci/render-scheduler-worker-endpoints.sh — DO NOT EDIT BY HAND.
apiVersion: v1
kind: ConfigMap
metadata:
  name: scheduler-worker-config
data:
  COGNI_NODE_ENDPOINTS: "canary=http://canary-node-app:3000,89612f02-114d-460d-87a5-c2ab212ccf6f=http://canary-node-app:3000,offcluster=https://offcluster-test.example.org,22222222-3333-4444-5555-666666666666=https://offcluster-test.example.org"
`;
    const out = insertSchedulerEndpoint(patchBefore, "ztest", NODE_ID);
    // The new (k3s) node appends last with the in-cluster convention...
    expect(out).toContain(
      `,ztest=http://ztest-node-app:3000,${NODE_ID}=http://ztest-node-app:3000"`
    );
    // ...and an already-off-cluster sibling keeps its public URL untouched.
    expect(out).toContain(
      "offcluster=https://offcluster-test.example.org,22222222-3333-4444-5555-666666666666=https://offcluster-test.example.org"
    );
  });
