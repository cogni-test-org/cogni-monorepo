 ...
 * Invariants: Schema array MUST list every operator-owned table — the @cogni/db-schema core glob plus each node-local schema file (nodes, node_access_requests, work_item_sessions, akash_tx_allocations); omitting one makes `db:generate` emit a destructive DROP and trips `db:check:generate-clean`. Migrations dir is operator-owned. DATABASE_URL must be provided by caller (pnpm scripts set it from .env.local / container env).
