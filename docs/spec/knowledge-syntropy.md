...
**The baseline every new node's dolt ships with** (grounded in the cognition bootstrap — `api/v1/cognition`):

- **6 generic tables** (per `docs/research/dolt-baseline-port-syntropy.md`): `knowledge`, `domains`, `sources`,
  `citations`, `knowledge_contributions`, `knowledge_contribution_commits` — the four core tables are detailed below;
  the two contribution tables + `work_items` live in `dolt-baseline-port-syntropy`.
- **Domains** — a small universal set + per-node niche. The model, the list, and the seeding path are owned by
  [knowledge-domain-registry § Seeding](./knowledge-domain-registry.md); not restated here.
- **Content boots empty** (`NODES_BOOT_EMPTY`); the one anchor seed is `<slug>-agent-orientation` (a `guide`).
  Everything else is EARNED via the EDO loop (below) + contributions.
...
| DOMAIN_REGISTRY_EXTENDS_VIA_UI           | Base domains are registered manually in v0 (`/knowledge` Domains UI / cookie-session `POST /api/v1/knowledge/domains`) — NOT by the schema migrator; automated seeding is a deferred dedicated step. `NODES_BOOT_EMPTY` scopes to content tables only. Contract: [knowledge-domain-registry](./knowledge-domain-registry.md).                                                                                                                                                                                                              |
