...
| Reconcile prod infra   | `POST /api/v1/deploy/infra-reconcile`     | `{ nodeId, env: "production" }`             | `node.promote_production` on the **operator node only**                                        | `200` + existing workflow dispatch; current app pin preserved |
...
A spawned node repo is **born protected** from the versioned policy owned by `Cogni-DAO/node-template` at `.cogni/repo-policy.json`. Node formation (`GitHubRepoWriter.forkFromTemplate`) App-reads that manifest from template `main`, validates the typed `cogni.node-repo-policy.v1` contract, and creates or exactly repairs the named default-branch ruleset on the fork. The manifest currently requires a pull request plus `unit`, `component`, `static`, and `manifest`, with no bypass actors. `scripts/check-node-ci-workflow.mjs` in node-template fails CI when any declared context is absent from `pull_request` or `merge_group`, so policy cannot drift from the checks the fork actually emits. Missing/invalid policy or a ruleset permission failure aborts formation loudly; the operator carries no second list of check names.
