...
- **`deploy-infra.sh` is NOT a DB-credential writer.** It brings up Compose infra and provisions DB roles, but the role passwords k8s pods authenticate with are OpenBao-owned ([`secrets-management.md`](./secrets-management.md) Invariant 15, `DB_ROLE_CREDS_ARE_OPENBAO_OWNED`). Provisioning creates each role **set-once**; it MUST NOT `ALTER` a pod-consumed role password from its rendered `.env` — a second writer that diverges from the ESO-synced Secret → `28P01` → 502 (bug.5002). This is the DB facet of Axiom 21's "generalizes to per-node secrets + DB." **As-built today** it still sets the role from a GH-secret-rendered `.env` (equal to OpenBao only by construction); migrating it to read those passwords from OpenBao via an in-cluster reader JWT — so there is genuinely one source — is the phased plan in [`secrets-management.md` → DB-credential provisioning](./secrets-management.md).

**Node-birth exception — one-flight birth (mochi gap).** Independence holds for ordinary flights, but a flight that _births_ a node is the one case where the app lever must also provision infra: a brand-new node's edge route + DB + secret don't exist on the VM yet, so app-only birth gets pods + DNS but the edge returns TLS-internal-error and the pod has no DB/secret. `candidate-flight.yml`'s `decide` job sets `needs_infra=true` when the PR `files` API reports an _added_ `infra/catalog/<slug>.yaml` (status `added` — the same authoritative source the affected-detector uses, immune to squash-merge merge-base pollution). On birth, an in-graph `deploy-infra` job (`needs: decide`, `if: needs_infra == 'true'`, `environment: candidate-a`) runs `scripts/ci/deploy-infra.sh --ref <head_sha>` — mirroring `candidate-flight-infra.yml`'s job but sourcing `infra/compose/**` from the PR head so the birthing PR's own infra lands. `verify-candidate` `needs:` it and gates `(deploy-infra.result == 'success' || 'skipped')`, mirroring `promote-and-deploy.yml`'s `verify-deploy`. On every non-birth (app-only) flight `deploy-infra` is **skipped** (visibly grey) and `verify-candidate` still runs — task.0314 independence is preserved; the coupling fires _only_ on birth. The standalone `candidate-flight-infra.yml` lever stays for infra-only and manual changes.
