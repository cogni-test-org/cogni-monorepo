...
- **One image per node — two layouts:** in-tree monorepo nodes build `ghcr.io/cogni-dao/cogni-template:{tag}-{operator,poly,resy}` from `nodes/<node>/app/Dockerfile` `runner` stage, app tree at `/app/nodes/<node>/app/`. Wizard-born **node-at-root** nodes build their own image (`ghcr.io/cogni-dao/<slug>`) from the node repo root, app tree at `/app/app/` (see [node-baas-architecture.md](./node-baas-architecture.md)). Either way the runner ships migration SQL + `migrate.mjs` alongside the standalone Next.js bundle (which carries `drizzle-orm` + `postgres` as production deps via `serverExternalPackages`); `next.config.ts` forces `outputFileTracingIncludes` of the full `drizzle-orm/**/*` + `postgres/**/*` packages so the bundle has their `package.json` + `exports` map.
- **Deployment initContainer:** `infra/k8s/base/node-app/deployment.yaml` declares `initContainers: [migrate]` using the same image as the main container, defaulting to the monorepo CMD `["/bin/sh", "-c", "exec node /app/nodes/$(NODE_NAME)/app/migrate.mjs /app/nodes/$(NODE_NAME)/app/migrations"]` (`NODE_NAME` from the `node-app-config` overlay patch). **The migrate command is part of the node's runtime shape and must match its image layout:** node-at-root overlays override it (and the `migrate-doltgres` command) to `/app/app/…` — the operator consumes the node's image, never assumes its internal path. The wizard's overlay generator emits these overrides (`nodes/operator/app/src/shared/node-app-scaffold/gens/overlay.ts` `NODE_AT_ROOT_MIGRATE_PATH` + `scripts/setup/scaffold-node.sh`); a path mismatch crash-loops migrate with `MODULE_NOT_FOUND` before any DB connect.
...
**Doltgres RBAC — why the knowledge plane connects as the `postgres` superuser (as-built, SSOT):**

Doltgres `0.56.3` implements only the five table-level DML privileges (`SELECT`, `INSERT`, `UPDATE`, `DELETE`, `TRUNCATE`), and **only for tables**. There is no `EXECUTE`-on-functions privilege (a non-superuser is denied `current_user`, `count()`, and any aggregate), no schema/`CREATE` privilege, no `ALTER DEFAULT PRIVILEGES`, no role membership, and `pg_roles` is unpopulated. `GRANT` returns success but enforcement is partial: a role that creates a table is then denied `INSERT`/`SELECT` on it until a superuser re-`GRANT`s, and the migrator cannot pre-grant for the tables it will create. Net: a per-node non-superuser `knowledge_<node>` role **cannot** run the drizzle migrator or serve the app, so every node's `DOLTGRES_URL` connects as the env `postgres` superuser (the `knowledge_reader`/`knowledge_writer` roles `provision.sh` creates exist but are effectively unused). This is an upstream limitation, not a Cogni defect — the underlying engine is Dolt's MySQL-compatible grant system surfaced over the pg wire (errors carry MySQL codes, e.g. `errno 1105 / sqlstate HY000`). **Secrets handling is identical to Postgres regardless** (OpenBao-sole-source per [secrets-management Invariant 15](secrets-management.md)); only the credential value-shape differs — env superuser today, swap to per-node `source: agent` the day Doltgres implements function/schema/role privileges, **zero pipeline change**. Verified empirically against the pinned `dolthub/doltgresql:0.56.3` image (2026-06-09). Sources: [DoltHub "Doltgres Now Supports Users" (2024-11-07)](https://www.dolthub.com/blog/2024-11-07-doltgres-supports-users/), [Dolt Access Management](https://docs.dolthub.com/sql-reference/server/access-management).
