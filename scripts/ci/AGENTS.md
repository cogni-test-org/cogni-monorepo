...
- **Exports:** none
- **CLI (if any):** `build.sh`, `push.sh`, `deploy.sh`, `deploy-infra.sh`, `bootstrap-openfga.sh`, `test-image.sh`, `promote-k8s-image.sh`, `flight-preview.sh`, `create-release.sh`, `validate-dsns.sh`, `ensure-temporal-namespace.sh`, `compute_migrator_fingerprint.sh`, `check-gitops-manifests.sh`, `check-gitops-service-coverage.sh`, `loki_push.sh`, `fetch_github_job_logs.sh`, `seed-pnpm-store.sh`, `detect-affected.sh`, `classify-env-manager-fast-path.sh`, `build-and-push-images.sh`, `write-build-manifest.sh`, `resolve-pr-build-images.sh`, `promote-build-payload.sh`, `promote-preview-seed-main.sh`, `aggregate-rollup.sh`, `aggregate-decide-outcome.sh`, `resolve-cell-state.sh`, `report-candidate-status.sh`, `wait-for-candidate-ready.sh`, `smoke-candidate.sh`, `wait-for-argocd.sh`, `wait-for-in-cluster-services.sh`, `verify-buildsha.sh`, `update-source-sha-map.sh`, `provision-grafana-postgres-datasources.sh`, `workflow-check.mjs`, `sync-node-template-fork-pr.sh`, `check-deploy-ref-ancestry.sh`, `detect-sync-drift.mjs`, `sync-test-parent.mjs`, `resolve-test-parent.mjs`
- **Env/Config keys:** `IMAGE_NAME`, `IMAGE_TAG`, `APP_IMAGE`, `MIGRATOR_IMAGE`, `COGNI_REPO_URL`, `COGNI_REPO_REF`, `PLATFORM`, `GHCR_PAT`, `CHERRY_AUTH_TOKEN`, `TF_VAR_*`, `POSTGRES_ROOT_USER`, `POSTGRES_ROOT_PASSWORD`, `APP_DB_USER`, `APP_DB_PASSWORD`, `APP_DB_SERVICE_USER`, `APP_DB_SERVICE_PASSWORD`, `APP_DB_READONLY_USER`, `APP_DB_READONLY_PASSWORD`, `APP_DB_NAME`, `DB_BACKUP_INTERVAL_SECONDS`, `DB_BACKUP_RETENTION_DAYS`, `OPENFGA_API_URL`, `OPENFGA_STORE_ID`, `OPENFGA_AUTHORIZATION_MODEL_ID`, `OPENFGA_AUTHORIZATION_MODEL_HASH`, `OPENFGA_API_TOKEN`, `OPENFGA_AUTHN_METHOD`, `OPENFGA_STORE_NAME`, `OPENFGA_MODEL_FILE`, `GRAFANA_URL`, `GRAFANA_SERVICE_ACCOUNT_TOKEN`, `GRAFANA_PDC_SIGNING_TOKEN`, `GRAFANA_PDC_HOSTED_GRAFANA_ID`, `GRAFANA_PDC_CLUSTER`, `GRAFANA_PDC_NETWORK_ID`, `GRAFANA_PDC_NETWORK_UUID`, `LOKI_URL`, `LOKI_USER`, `LOKI_TOKEN`, `INTERNAL_OPS_TOKEN`, `LOG_FILE`, `JOB_NAME`, `LABELS`, `GITHUB_TOKEN`, `GITHUB_REPOSITORY`, `GITHUB_RUN_ID`, `GITHUB_RUN_ATTEMPT`, `GITHUB_JOB`, `OUTPUT_FILE`, `TARGETS`, `TURBO_BIN`, `TURBO_SCM_BASE`, `TURBO_SCM_HEAD`, `PR_NUMBER`, `HEAD_SHA`, `RUN_ID`, `RUN_ATTEMPT`, `WORKFLOW_NAME`, `REF_NAME`, `GHCR_USERNAME`, `GHCR_TOKEN`, `IMAGES_FILE`, `MANIFEST_FILE`, `PAYLOAD_FILE`, `OVERLAY_ENV`, `SLOT`, `LEASE_FILE`, `STATUS_URL`, `TTL_MINUTES`, `STATE`, `DESCRIPTION`, `TARGET_URL`, `CONTEXT`, `DOMAIN`, `DNS_RECONCILE_SUMMARY_FILE`, `DNS_RECONCILE_CANDIDATE_SHA`, `DNS_RECONCILE_HEAD_SHA`, `DNS_RECONCILE_NODE_SOURCE_SHA`, `DNS_RECONCILE_PR_NUMBER`, `DNS_RECONCILE_NODE_SLUG`, `DNS_RECONCILE_STATUS_URL`, `MAX_ATTEMPTS`, `SLEEP_SECONDS`, `VM_HOST`, `DEPLOY_ENVIRONMENT`, `EXPECTED_SHA`, `EXPECTED_BUILDSHA`, `NODES`, `SOURCE_SHA_MAP`, `ARGOCD_SYNC_VERIFIED`, `MAP_FILE`, `MAP_SCRIPT`, `PROMOTED_APPS`, `ARGOCD_TIMEOUT`, `ACTIVE_SYNC_AFTER`, `SSH_OPTS`, `SSH_KEY`, `CI_SSH_RETRY_ATTEMPTS`, `ROLLOUT_TIMEOUT`, `TEMPLATE_REPO`, `BASE_BRANCH`, `PR_TITLE`, `WATCH`, `WORK_ROOT`
- **Fast-path classifier env:** `EVENT_NAME`, `REPOSITORY`, `PR_NUMBER_PR`, `PR_HEAD_SHA_PR`, `MQ_HEAD_REF`, `MQ_BASE_SHA`, `MQ_HEAD_SHA`; `FAST_PATH_{PR,COMMIT,FILES}_JSON`, `FAST_PATH_{BASE,HEAD}_CATALOG`, and `FAST_PATH_APPSET_PATHS_LIB` are hermetic test seams only
- **Files considered API:** `scripts/*.sh`, `workflow-check.mjs`, `infra/openfga/rbac-model.json`
...
- `deploy-infra.sh` runs `bootstrap-openfga.sh` against `infra/openfga/rbac-model.json` after the OpenFGA runtime is healthy, then publishes the resolved store/model IDs through OpenBao/ESO for operator pods. `OPENFGA_API_TOKEN` is optional unless OpenFGA authn is enabled.
...
- `check-deploy-ref-ancestry.sh` (bug.5150) enforces `DEPLOY_REF_SOURCE_IS_REACHABLE_FROM_MAIN` in its own `deploy-ref-ancestry` CI job. A deploy ref's TIP is never on main in either shape the fleet uses, so a bare `merge-base --is-ancestor <tip> origin/main` carries zero signal; the script first resolves the ref's monorepo **source** commit — the `Reviewed-Source:` trailer the operator writes on a control-plane selection, else `.promote-state/source-sha-by-app.json` keyed by the ref's **own** app (`ROLLUP_MAP_PRESERVES_UNAFFECTED`), else the tip — then asserts ancestry on that. Jurisdiction: `source-selection` (control-plane refs; `AHEAD` is a live flight and passes with a loud WARN, `DIVERGED`/`UNREACHABLE` fail) and `promoted-app` (preview/production, monorepo-built; `ON_MAIN` only). `flight-slot` (candidate-a app refs — the pre-merge slot is off-main by design), `foreign-source` (remote-source nodes, whose SHA belongs to another repo), `no-provenance` and `absent` (a born node never flighted, which must not fail the build) are printed with their reason, never silently dropped. Failures name the ref, tip, source SHA, state, and the sanctioned fix verb `POST /api/v1/deploy/infra-reconcile`. Enumeration is the catalog (`deploy_branch_for_target`) plus non-templated `targetRevision`/`revision` values under `infra/k8s/argocd/`; costs one `ls-remote`, one `--depth=1` tip fetch, and a round trip per off-main asserted source. Tested by `scripts/ci/tests/check-deploy-ref-ancestry.test.sh` (hermetic local-path origin, includes the RED case).
...
- `deploy-infra.sh` installs one systemd-owned `db-backup` executor. Candidate infra flights run the full backup as behavioral validation; preview/production reconciles only install/start the timer and never launch a full dump inline. `scripts/ci/tests/render-caddyfile.test.sh` guards the serialization, environment gate, client resource bounds, and PostgreSQL/kernel log evidence (bug.5252).
...
- `deploy.sh` uses targeted pulls: only per-deploy images (app, migrator, scheduler-worker) and sandbox `:latest` images (pnpm-store) are explicitly pulled. The `:latest` pulls do a manifest check (~2s) and skip download if unchanged. Static/pinned images (postgres, litellm, alloy, temporal, autoheal, nginx, git-sync, busybox) use local Docker cache and are pulled by `compose up -d` only when missing.
