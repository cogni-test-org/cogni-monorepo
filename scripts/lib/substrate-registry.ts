...
export const SUBSTRATE_DEPENDENCIES: readonly SubstrateDependency[] = [
  ...
  },
  {
    ...
    reconcile: {
      ...
      note: "TWO HALVES, both now wired (task.5026). DELIVERY: a per-env-singleton flight job propagates the catalog-rendered base/scheduler-worker (the all-k3s placement default) PLUS this env's generated overlays/<env>/scheduler-worker/node-endpoints.patch.yaml (the provider-resolved map — bug.5094), both gated against catalog by render-scheduler-worker-endpoints.sh --check, onto deploy/<env>-scheduler-worker — the branch the worker's Argo app actually syncs, which a node-add never refreshed before (this was the manual heal). ROLL: base/scheduler-worker/deployment.yaml carries reloader.stakater.com/auto (#1609), so Argo syncing the changed ConfigMap auto-rolls the worker. Idempotent (no-op when the routing CSV is unchanged); the env overlay's pinned digest is never touched. assertLive proves the worker actually polls the new node's queue (a graph run completes).",
    },
